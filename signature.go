...
-----END RSA PRIVATE KEY-----`
	...
	//图文消息
	ImgText = "{'articles':[{'actionName':'立即查看','desc':'这是图文内容','imageUrl':'http://pic.alipayobjects.com/e/201311/1PaQ27Go6H_src.jpg','title':'这是标题','url':'https://www.alipay.com/'}],'msgType':'image-text','toUserId':'xrHhCfv4l5ccveXOzmo7ipqKo7ILToCxhO4skMe5bRK-S4-HyunMvYTqiRYEUD+U01'}"

	//模板消息
	templateText = "{" +
		"   'toUserId':'xrHhCfv4l5ccveXOzmo7ipqKo7ILToCxhO4skMe5bRK-S4-HyunMvYTqiRYEUD+U01'," +
		"   'template':{" +
...
func signature() {
	...
	hashed := sha1.Sum([]byte(fmt.Sprintf(requestBody, templateText, methodTEM, timestamp)))
	sign, err := rsa.SignPKCS1v15(rand.Reader, key, crypto.SHA1, hashed[:])

	//base64
	signature := base64.StdEncoding.EncodeToString(sign)
	postMsg(signature, timestamp)
}

func postMsg(signature, timestamp string) {
	...
	q.Set("biz_content", templateText)
	q.Set("charset", "GBK")
	...
	resp, err := http.Post(u.String(), "POST", bytes.NewBufferString("test"))
